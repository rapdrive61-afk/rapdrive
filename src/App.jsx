...
const STATUS = {
  ...
  visited:   {label:"Entregado", color:"#10b981", bg:"rgba(16,185,129,0.12)"},
...
const PageRoutes = () => {
  ...
  const routeStats = (r) => {
    ...
    const pending   = stops.filter(s => s.navStatus !== "visited" && s.navStatus !== "not_delivered").length;
                ...
                return (
                  <div key={stop.id||i} style={{ display:"flex", gap:12, padding:"10px 16px", borderBottom:"1px solid #080e16", alignItems:"flex-start" }}>
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      ...
                      {stop.visitedAt && <div style={{ fontSize:10, color:"#10b981", marginTop:2 }}>Entregado {stop.visitedAt}</div>}
          ...
          <>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:2 }}>
              {[
                ...
                { key:"onDelivered",     label:"Parada visitada",             desc:"Cuando un mensajero marca entregado" },
...
const motorSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="34" height="34" viewBox="0 0 34 34">
  ...
  useEffect(() => {
    ...
    const makeFixedPinIcon = (stop) => {
      const isDone = stop.navStatus === "visited";
      const isFailed = stop.navStatus === "not_delivered";
      const label = String(stop.stopNum || "?");
      const cacheKey = `${isDone ? "done" : isFailed ? "failed" : "pending"}:${label}`;
      if (pinIconCacheRef.current[cacheKey]) return pinIconCacheRef.current[cacheKey];

      const color = isDone ? "#22c55e" : isFailed ? "#ef4444" : "#2563eb";
      const dark  = isDone ? "#16a34a" : isFailed ? "#b91c1c" : "#1d4ed8";
  ...
  const isDeliveredStop = (s) => s?.navStatus === "visited";
  const isFailedStop = (s) => s?.navStatus === "not_delivered";
  const isClosedStop = (s) => isDeliveredStop(s) || isFailedStop(s);
  const visited   = routeStops.filter(isDeliveredStop);
  const problems    = routeStops.filter(isFailedStop);
  const pending     = routeStops.filter(s=>!isClosedStop(s));
  const currentStop = routeStops.find(s=>s.navStatus==="active") || routeStops.find(s=>!isClosedStop(s));
  ...
  const pushUpdate = (updatedStops) => {
    ...
    const allDone = updatedStops.length > 0 && updatedStops.every(
      s => s.navStatus === "visited" || s.navStatus === "not_delivered"
    );
  ...
  const advanceAfterClosingStop = (updatedStops) => {
    return updatedStops.map((s, i, arr) => {
      if (s.navStatus === "visited" || s.navStatus === "not_delivered") return s;
      const firstPendingIndex = arr.findIndex(x => x.navStatus !== "visited" && x.navStatus !== "not_delivered");
  ...
  const markStopVisited = (stop) => {
    ...
    const closedStops = stops.map(s => {
      const same = (s.id && stop.id && s.id === stop.id) || (s.stopNum && stop.stopNum && s.stopNum === stop.stopNum);
      if (!same) return s;
      return { ...s, navStatus:"visited", visitedAt: s.visitedAt || now, failedAt: undefined };
    });
    const updatedStops = advanceAfterClosingStop(closedStops);
  ...
  const markStopFailed = (stop) => {
    if (!stop || stop.navStatus === "not_delivered") return;
    const now = new Date().toLocaleTimeString("es-DO", { hour:"2-digit", minute:"2-digit" });
    const closedStops = stops.map(s => {
      const same = (s.id && stop.id && s.id === stop.id) || (s.stopNum && stop.stopNum && s.stopNum === stop.stopNum);
      if (!same) return s;
      return { ...s, navStatus:"not_delivered", failedAt: s.failedAt || now, visitedAt: undefined };
    });
    const updatedStops = advanceAfterClosingStop(closedStops);
    setStops(updatedStops);
    const nextSelected = updatedStops.find(s => (s.id && stop.id && s.id === stop.id) || (s.stopNum && stop.stopNum && s.stopNum === stop.stopNum));
                ...
                <div style={{ padding:"11px 13px 13px" }}>
                  ...
                  <div style={{ display:"flex", alignItems:"flex-start", gap:9, marginBottom:10 }}>
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      ...
                      {/* Estado si entregado/note */}
                      {(isDone || isProb) && (
                        <div style={{ fontSize:10, color:ac, fontWeight:600, marginTop:2 }}>
                          {isDone ? `✓ Entregado${s.visitedAt ? " · " + s.visitedAt : ""}` : `⚠ Nota`}
                        </div>
                      )}
                    </div>
        ...
        <div
          ...
          <div ref={listScrollRef} style={{ flex:1,overflowY:"auto",overflowX:"hidden",paddingBottom:8,marginTop:4,WebkitOverflowScrolling:"touch" }}>
            ...
            {filteredStops.map((stop,i) => {
              ...
              const statusLabel= isDone?"ENTREGADO":isProb?"FALLIDO":isEnR?"EN CAMINO":"PENDIENTE";
              ...
              return (
                <div key={stop.id} style={{
                  ...
                  <div onClick={()=>{ setSelStop(isExp?null:stop); if(!isExp&&gMapRef.current&&stop.lat&&stop.lng){gMapRef.current.panTo({lat:stop.lat,lng:stop.lng});gMapRef.current.setZoom(16);} }}
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      {/* Nombre cliente */}
                      <div style={{ fontSize:13.5, fontWeight:700, color: isDone?"rgba(255,255,255,0.55)":isProb?"rgba(255,255,255,0.52)":isCur?"#ffffff":"rgba(255,255,255,0.85)", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap", letterSpacing:"-0.2px", lineHeight:1.2 }}>
                      ...
                      {isDone && stop.visitedAt && (
                        <div style={{ fontSize:10, color:"#10b981", marginTop:4, fontFamily:"'DM Mono',monospace", display:"flex", alignItems:"center", gap:4 }}>
                          ...
                          Entregado · {stop.visitedAt}
                        </div>
                      )}
                      {isProb && stop.failedAt && (
                        <div style={{ fontSize:10, color:"#ef4444", marginTop:4, fontFamily:"'DM Mono',monospace", display:"flex", alignItems:"center", gap:4 }}>
                  ...
                  return (
                    <div key={stop.id||i} style={{ display:"grid", gridTemplateColumns:"38px 1fr", gap:10, padding:"10px 10px", marginBottom:8, borderRadius:16, background:"rgba(255,255,255,.035)", border:"1px solid rgba(148,163,184,.08)" }}>
                      ...
                      <div style={{ minWidth:0 }}>
                        ...
                        {stop.visitedAt && <div style={{ marginTop:5, fontSize:10.5, color:"#86efac", fontWeight:750 }}>Entregado · {stop.visitedAt}</div>}
                      </div>
                    </div>
                  );
                })}
        ...
        if (anyChange && !snap.routeCompletedNotified) {
          ...
          if (allDone) {
            ...
            pushEvent({ id:"e"+Date.now()+"done"+driverId, type:"visited", icon:"🏁", color:"#10b981",
              ...
              body:`${del} entregados · ${prob} notas · ${route.stops.length} paradas totales`,
              time: now, read: false, isNew: true });
          }
        }
      });
