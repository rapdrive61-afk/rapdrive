...
const AdminMotorV5 = () => {
  const [test,setTest]=useState(''), [score,setScore]=useState(null);
  const runTest=()=>{ const q=test.trim().toLowerCase(); if(!q) return; const bonus=(q.includes('herrera')||q.includes('alcarrizo')||q.includes('km')||q.includes('duarte')||q.includes('manoguayabo'))?18:0; setScore(Math.min(98,76+bonus+Math.min(4,q.length))); };
  return <div style={{flex:1,background:v5.bg,color:v5.text,overflow:'auto'}}><div style={{padding:26,borderBottom:`1px solid ${v5.line}`,display:'flex',justifyContent:'space-between',alignItems:'center'}}><div><div style={{color:v5.blue,fontSize:11,fontWeight:1000,letterSpacing:'2px',textTransform:'uppercase'}}>Address intelligence</div><h1 style={{margin:'6px 0 4px',fontSize:30,fontWeight:1000,letterSpacing:'-.9px'}}>Motor</h1><div style={{color:v5.soft,fontSize:13}}>Autocorrección, referencias locales y creación de rutas desde Excel.</div></div></div><div style={{padding:24,display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:14}}><V5Card title="Referencias" value="500+" sub="SDO · Alcarrizos · DN" color={v5.blue} icon={<V5Icon type="pin"/>}/><V5Card title="Precisión meta" value="98%" sub="Motor local + Google" color={v5.green} icon={<V5Icon type="chart"/>}/><V5Card title="Sinónimos" value="120+" sub="km9, prol, alcarizo..." color={v5.violet} icon={<V5Icon type="engine"/>}/><V5Card title="Cache" value="Local" sub="Respuesta inmediata" color={v5.amber} icon={<V5Icon type="depot"/>}/></div><div style={{padding:'0 24px 24px',display:'grid',gridTemplateColumns:'1fr 420px',gap:18}}><div style={{background:v5.panel,border:`1px solid ${v5.line}`,borderRadius:22,overflow:'hidden'}}><div style={{padding:20,borderBottom:`1px solid ${v5.line}`}}><div style={{fontSize:18,fontWeight:1000}}>Laboratorio de direcciones</div><div style={{color:v5.soft,fontSize:13,marginTop:5}}>Prueba cómo responde el corrector antes de importar rutas.</div></div><div style={{padding:20}}><div style={{display:'flex',gap:10}}><input value={test} onChange={e=>setTest(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')runTest()}} placeholder="Ej. km9 autopista duarte, manoguayabo, pueblo chico..." style={{flex:1,background:'#06101d',border:`1px solid ${v5.line}`,borderRadius:14,padding:'14px 16px',color:v5.text,outline:0,fontSize:14}}/><button onClick={runTest} style={{border:0,borderRadius:14,padding:'0 18px',background:v5.blue,color:'white',fontWeight:1000,cursor:'pointer'}}>Probar</button></div>{score&&<div style={{marginTop:18,display:'grid',gridTemplateColumns:'130px 1fr',gap:14,alignItems:'center'}}><div style={{width:116,height:116,borderRadius:'50%',display:'grid',placeItems:'center',background:`conic-gradient(${score>90?v5.green:v5.amber} ${score*3.6}deg,#07101b 0deg)`,fontSize:26,fontWeight:1000}}>{score}%</div><div><div style={{fontWeight:1000,fontSize:17}}>Coincidencia estimada</div><div style={{color:v5.soft,fontSize:13,marginTop:7,lineHeight:1.5}}>El motor prioriza referencias locales, sectores y sinónimos antes de consultar Google.</div></div></div>}</div></div><div style={{background:v5.panel,border:`1px solid ${v5.line}`,borderRadius:22,padding:20}}><div style={{fontSize:18,fontWeight:1000,marginBottom:16}}>Pipeline del motor</div>{[['1','Normaliza texto','Quita acentos, abreviaturas y errores comunes'],['2','Busca local','Dataset propio de referencias SDO / DN'],['3','Rankea','Puntúa sector, calle, referencia y cercanía'],['4','Fallback','Google solo si el motor local no resuelve']].map(([n,t,s])=><div key={n} style={{display:'flex',gap:12,padding:'13px 0',borderBottom:`1px solid ${v5.line}`}}><div style={{width:32,height:32,borderRadius:10,background:'rgba(59,130,246,.14)',color:v5.blue,display:'grid',placeItems:'center',fontWeight:1000}}>{n}</div><div><div style={{fontWeight:1000}}>{t}</div><div style={{color:v5.muted,fontSize:12,marginTop:3}}>{s}</div></div></div>)}</div></div><div style={{padding:'0 24px 28px'}}><div style={{border:`1px solid ${v5.line}`,borderRadius:22,overflow:'hidden',background:v5.panel}}><CircuitEngine/></div></div></div>;
};
